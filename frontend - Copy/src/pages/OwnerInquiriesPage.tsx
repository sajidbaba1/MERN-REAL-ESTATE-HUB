...
import { Loader2, MessageSquare, Calendar, DollarSign, Home, AlertCircle, Users, Bell } from 'lucide-react';
...
const OwnerInquiriesPage: React.FC = () => {
  ...
  const getStatusCounts = () => {
    const counts = inquiries.reduce((acc, inquiry) => {
      ...
      return acc;
    }, { total: inquiries.length } as { [key: string]: number });
            ...
            <nav className="-mb-px flex space-x-8">
              {['ALL', 'ACTIVE', 'NEGOTIATING', 'AGREED', 'PURCHASED', 'CANCELLED'].map((status) => (
                <button
                  ...
                  className={`py-2 px-1 border-b-2 font-medium text-sm ${statusFilter === status
          ...
          <div className="space-y-6">
            {filteredInquiries.map((inquiry) => {
              ...
              return (
                <div
                  key={inquiry.id}
                  className={`bg-white rounded-xl shadow-sm border-l-4 hover:shadow-md transition-shadow ${priority === 'high' ? 'border-l-green-500' :
                            ...
                            <div className="bg-gray-50 rounded-lg p-3 mb-3">
                              ...
                              <div className="flex items-center space-x-4 text-sm text-gray-600">
                                ...
                                {inquiry.client.phoneNumber && (
                                  <a href={`tel:${inquiry.client.phoneNumber}`} className="text-blue-600 hover:underline">
                                    {inquiry.client.phoneNumber}
                                  </a>
                                )}
                              </div>
