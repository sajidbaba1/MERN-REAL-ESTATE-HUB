...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (useOtp) {
      // OTP login flow
      if (!formData.email) {
        showToast.error('Please enter your email to receive an OTP');
        return;
      }
      if (!otpSent) {
        showToast.error('Please request an OTP first');
        return;
      }
      ...
      try {
        ...
        showToast.error(typeof msg === 'string' ? msg : 'OTP verification failed. Please try again.');
      } finally {
        setOtpLoading(false);
      }
    } else {
      ...
      try {
        ...
        showToast.error(typeof msg === 'string' ? msg : 'Login failed. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data;
      showToast.error(typeof msg === 'string' ? msg : 'Failed to send OTP');
    } finally {
      setOtpLoading(false);
    }
  };
  ...
  const quickLogin = async (role: 'ADMIN' | 'AGENT' | 'USER') => {
    try {
      ...
      showToast.error('Demo login failed. Ensure backend is running and demo users are seeded.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div className="text-center text-sm text-gray-600">
            {useOtp ? (
              <>
                Prefer password?{' '}
                <button type="button" className="text-primary-600 hover:text-primary-700 font-medium" onClick={() => setUseOtp(false)}>
              ...
              <>
                No password?{' '}
                <button type="button" className="text-primary-600 hover:text-primary-700 font-medium" onClick={() => setUseOtp(true)}>
                  Login with OTP
                </button>
              </>
            )}
