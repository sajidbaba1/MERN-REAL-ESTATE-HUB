...
const API_BASE_URL = (import.meta as any).env.VITE_API_BASE_URL || 'http://localhost:8889/api';
...
export const propertyApi = {
  ...
  // Update property
  updateProperty: async (id: string, property: Omit<Property, 'id' | 'createdAt' | 'updatedAt'>): Promise<Property> => {
  ...
  claimOwnership: async (id: string): Promise<Property> => {
  ...
  // Get filter metadata (consolidated)
  getFilterMeta: async (): Promise<{ cities: string[]; states: string[]; minPrice: number; maxPrice: number }> => {
    const response = await api.get('/properties/filters/meta');
    return response.data;
  },

  // Specialized legacy methods if still needed by some components
